...
This document describes the process of getting the N-Pron experiment (Novermber 29, 2014) data (i.e. the ProsodyPro output) into R and begin processing, visualizing and doing analysis with it. It assumes that you've already created the ProsodyPro "ensemble" files. To do this with proper "across item" and "across speaker" averaging see the document, ***How to create ensemble files with PProsodyPro 6.1.3 beta.Rmd***.
...
Extract the `mean_normf0` data.frame to `dat` for safer and easier processing:
```
...
### Add a variable, g(loss), for the noun of each item

```
g <- gsub("_(.*)_de_(.*)","\\1", dat$X1) # extract the noun from the filename in the                                                         X1 (Normtime) variable
g <- gsub("_(.*)_","\\1", g) # extract "_" from entries that lacked a pronoun
dat$g <- g
table(dat$g) # inspect
```
...
```
pron <- gsub("_(.*)_de_(.*)_","\\2", dat$X1) # extract the pronoun from the filename
pron <- gsub("_.*","NA", pron) # change bare nouns to NA (i.e item without a pron) 
dat$pron <- pron
...
#### Read in list of nouns with melodies from `N-Pron noun melody list.csv`
...
#### Copy ```dat$g``` to new variables ```dat$mix``` and ```dat$mel```
