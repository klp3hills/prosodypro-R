...
This document describes the process of getting the Complex Noun production experiment (May 28-29, 2015) data (i.e. the ProsodyPro output) into R and begin processing, visualizing and doing analysis with it. 
...
The first step is to get the data into R using the script `get_ensemble_files.R`. This R script imports "ensemble" output files from the [ProsodyPro](http://www.homepages.ucl.ac.uk/~uclyyix/ProsodyPro/) Praat script developed by Yi Xu. These files are aggregate calculations for all tokens of each wordlist item for each speaker of various acoustic properties (e.g. f0, duration, intensity, etc).

Background:

* The ProsodyPro 6.0 beta2 script must be run with *Task #4: Get ensemble files* selected and the *Keep files together* box checked **after** running *Task #3: Process all sentences* with the *Keep files together* box checked
* All ProsodyPro "ensemble" output files are `.txt` files (e.g. mean_normf0.txt, maxf0.txt, minf0.txt, etc.)
* Each speaker's files must be in a separate folder (directory) named with their 3 initials (which will be added to the dataframes created as the sp(eaker) variable
* All speaker folders (directories) must be in the same enclosing folder (directory) which must be manually assigned to sp.dir before running the script
...
In order for this to work, there must be an observation (row) for each of the wordlist items. If an item is missing, delete that item from melody.list, or the script won't work. Initially, when processing just MQM data, there was no data for 'ceñidor para mujer', so I had to delete this from `melody.list`, so I stored the original wordlist data in `melody.list.original`:
...
Once this list is correct, execute this code to substitue the proper Mixtec word, melody and ipa from ```melody.list``` for each noun in ```dat$mix```, ```dat$mel``` and ```dat$ipa```:
